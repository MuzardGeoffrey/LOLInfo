namespace LOLInfo.Tests.CdragonModel.Parts
{
    ...
    public class StatScalingPartTests
    {
        ...
        {
            // mStat=0 (AP), mStatFormula=2 (Total), ratio=0.45 → "45% PA" (le '+' vient de la jointure)
            var part = new StatScalingPart(0, 2, "APRatio", DataWith("APRatio", 0.45));
        ...
        {
            // mStat=2 (AD), mStatFormula=1 (Bonus), ratio=0.60 → "60% AD bonus"
            var part = new StatScalingPart(2, 1, "ADRatio", DataWith("ADRatio", 0.60));
        ...
        public void Format_MultipleRanks_ShowsSlashSeparated()
        {
            ...
            var part = new StatScalingPart(0, 2, "APRatio", DataWith("APRatio", 0.3, 0.4, 0.5));
        ...
        {
            var part = new StatScalingPart(0, 2, "Missing", DataWith("Other", 0.5));
        ...
        {
            var part = new StatScalingPart(0, 2, "x", DataWith("x", 0.5));
        ...
        {
            var part = new StatScalingPart(0, 0, "x", DataWith("x", 0.5));
